...
import { User, LogOut, PanelLeftOpen } from 'lucide-react';
...
const HeaderPanel = () => {
    ...
    const {showSidebar, toggleSidebar, showNavigation} = useAppContext();
    ...
    return (
        <div className="flex justify-between items-center bg-white border-b px-4 pl-3 relative max-h-[56px] min-h-[56px] h-[56px] z-[999]">
            {!showSidebar && <div onClick={()=>toggleSidebar(true)} className="flex z-[1001] items-center justify-center w-[35px] h-[35px] rounded-md border border-slate-200 hover:bg-gray-100 my-auto mr-1 text-slate-400 hover:text-slate-700">
            ...
            <div className="flex items-center space-x-3 border-l pl-5 ml-auto">
                ...
                <div className="relative" ref={dropdownRef}>
                    <div onClick={toggleDropdown} className="cursor-pointer flex items-center justify-center w-8 h-8 bg-rose-400 rounded-full text-white">
